在shuhao1022/Navi-Link 的基础上，重新修改投屏界面，实现功能 灵动岛模块自定义+巡航模块自定义，即HUD+仪表盘可以自定义，主屏幕未做任何修改。

所有修改的代码遵循原作者代码不变，进行UI布局和交互修改，映射和功能均不受影响。

该项目均由DeepSeek V4操盘。

2026.9.1 UI布局制作完成，数据交互（待验证）
2026.9.2 交互验证，数据互通
2026.9.3 方向变更 该项目废弃 另外迁移至Antigravit,安卓更友好
...
- **启动方式**：只启动服务 / 正常打开配置页
- **主题色**：13种预设颜色可选（黑/蓝/浅蓝/橙/粉红/紫/深橙/青绿/朱红/翡翠/靛蓝/琥珀/棕色），自动计算文字对比度，启动方式卡片和样式卡片同步跟随
...
- **设备硬件诊断**：
  ...
  - **显示信息**：展示屏幕物理分辨率、应用可用分辨率、屏幕像素密度（dpi及缩放因子）以及屏幕刷新率。
